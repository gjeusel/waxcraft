...
- `defaultThinkingLevel: "high"` — initial level for new sessions. Cycle at runtime
  ...
  tokens). Runtime changes stay session-local via `pi-builtin-adjustments/index.ts`.
...
## extensions/pi-builtin-adjustments/index.ts

Collects three small adjustments to Pi's built-in behavior:

- Keeps model and thinking-level changes session-local by suppressing writes to
  `defaultProvider`, `defaultModel`, and `defaultThinkingLevel`; session history
  still records and restores active selections.
- Suppresses startup `No models match pattern` warnings for optional providers.
  Only `claude-bridge/*` and `openai-codex/*` remain mandatory and retain warnings.
- Rewrites the verbose exit hint to the single dim-gray `pi --session <id>` line
  using a narrowly scoped stdout interceptor during a real quit.
...
by `pi-builtin-adjustments/index.ts`) — through
