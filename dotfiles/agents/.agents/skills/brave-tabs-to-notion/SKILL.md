...
Use ONLY the `notion-personal` MCP server (tools prefixed `mcp__notion-personal__`). Load its tools via ToolSearch if not yet available.

NEVER use the `claude_ai_Notion` connector tools — that connector is authorized against the WORK workspace. If `notion-personal` tools fail with an auth error, stop and tell the user to run `/mcp` and authenticate `notion-personal` against their personal workspace.
...
5. **Dispatch** with `notion-personal` tools:
   - For the two databases: `notion-fetch` the database first to learn its data-source schema, then create one entry per link mapping title → title property and URL → the URL property (match whatever the schema actually has; leave other properties at defaults).
   - For the Watch List page: `notion-fetch` it first to see its structure, then append links in the same style the page already uses (fallback: a bulleted list of `[title](url)` under today's date).
